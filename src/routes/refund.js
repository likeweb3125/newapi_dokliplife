 ...
 *   post:
 *     summary: 환불 및 퇴실처리
 *     description: "계약에 대한 환불 및 퇴실처리를 수행합니다. 환불 요청을 il_room_refund_request에 저장합니다 (totalRefundAmount 0원 포함). roomStatus를 CHECKOUT_CONFIRMED로, extraPayment는 paymentStatus=FIN, parkStatus는 ENDED로 변경하고, gosiwonParking autoUse/bikeUse를 동기화합니다. totalRefundAmount가 0이면 roomContract.status=CANCEL, 0 초과면 cancelReason 기준(FIN/CANCEL)으로 설정합니다. 취소사유 (FULL: 만기퇴실, INTERIM: 중도퇴실, CANCEL: 계약취소, ETC: 기타), liabilityReason: 귀책사유 (OWNER: 사장님, OCCUPANT: 입실자). check_basic_sell이 true인 경우 기본 판매 설정을 사용하고, false인 경우 unableCheckInReason이 있으면 BEFORE_SALES 상태로 설정하며, 없으면 사용자 지정 날짜를 사용합니다."
 *     tags: [환불관리]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *               totalRefundAmount:
 *                 type: integer
 *                 description: "총환불금액. 0원 포함 항상 il_room_refund_request에 저장됨. 0원이면 roomContract.status=CANCEL로 설정"
 *                 example: 70000
 *               directRefundConfirm:
 *                 type: boolean
...
// Swagger 비노출 (내부용) — POST /v1/refund/refundInsert
 ...
 *   get:
 *     summary: 환불 요청 목록 조회
 *     description: '환불 요청 목록을 조회합니다. rrr_payment_amt가 0인 건은 제외됩니다. DataTables 형식으로 응답하며, 고시원 ID로 필터링할 수 있습니다.'
 *     tags: [환불관리]
 *     security:
 *       - bearerAuth: []
 *     parameters:
