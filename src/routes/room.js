 ...
 *     description: "roomStatus에서 statusEndDate가 기준일(또는 당일) 이전인 건을 대상으로, CHECKOUT_CONFIRMED인 경우 il_room_deposit(해당 방·계약자명·계약자 전화번호)의 rdp_return_dtm 여부에 따라 status를 END_DEPOSIT(미반환) 또는 END(반환완료)로, CONTRACT인 경우 OVERDUE로 업데이트합니다. 매일 00:05 스케줄러는 당일 기준으로 자동 실행됩니다."
 *     tags: [Room]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *                     overdueUpdated:
 *                       type: integer
 *                       description: "CONTRACT → OVERDUE로 업데이트된 건수"
 */
 ...
 *                           nowStatus:
 *                             type: string
 *                             description: '방 현재 상태 (roomStatus.status, 예: ON_SALE, CONTRACT 등)'
 *                           month:
 *                             type: string
 ...
 *                     inUse:
 *                       type: integer
 *                       description: 이용중 (status = CONTRACT)
 *                       example: 300
 *                     overdue:
 *                       type: integer
 ...
 *                 type: string
 *                 enum: [Y, N]
 *                 description: 방 월비용 사용 여부 (Y=사용, N=미사용)
 *                 example: Y
 *     responses:
 *       200:
