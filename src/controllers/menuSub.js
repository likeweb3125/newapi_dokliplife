...
exports.postSubCategoryCreate = async (req, res, next) => {
	...
	try {
		...
		const mainBannerFilePath =
			mainBannerFile && mainBannerFile[0]
		...
		switch (parseInt(c_content_type)) {
			case enumConfig.contentType.HTML[0]:
				const processedHtmlContents =
					await utilMiddleware.base64ToImagesPath(
						content
					);
			...
			case enumConfig.contentType.QNA[0]:
				...
				subCategory = await i_category_board.findOrCreate({
					...
					defaults: {
						...
						b_top_html:
							processedTopHtmlContents.temp_contents,
						b_template: b_template,
						b_template_text:
							processedTemplateContents.temp_contents,
					},
				});
				break;
			default:
	...
	try {
		...
		switch (menuView.c_content_type) {
			...
			case enumConfig.contentType.QNA[0]:
				...
				menuObj.b_thumbnail_height =
					subView.b_thumbnail_height;
...
exports.putSubCategoryUpdate = async (req, res, next) => {
	...
	try {
		...
		const getFile = (fieldName, currentPath) => {
			...
			return file && file[0]
				? currentPath !== file[0].path &&
				  currentPath !== null
					? (multerMiddleware.clearFile(currentPath),
					  file[0].path)
					: file[0].path
				: currentPath;
		};
		...
		const menuOnImgPath = getFile(
			'c_menu_on_img',
			menuView.c_menu_on_img
		);
		const menuOffImgPath = getFile(
			'c_menu_off_img',
			menuView.c_menu_off_img
		);
		...
		if (c_main_banner_file_del === 'Y') {
			try {
				...
				await multerMiddleware.clearFile(
					menuView.c_main_banner_file
				);
			} catch (err) {
		...
		switch (parseInt(c_content_type)) {
			...
			case enumConfig.contentType.QNA[0]:
				...
				const boardView =
					await i_category_board.findOrCreate({
						where: {
							parent_id: id,
							use_yn: enumConfig.useType
								.Y[0],
