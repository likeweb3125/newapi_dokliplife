...
const { i_category, sequelize } = require('../models');
            ...
            .map((item) => {
               ...
               if (submenu.length > 0) {
                  return {
                     ...
                     c_content_type: mapContentType(item.c_content_type),
                     submenu,
                  };
               } else {
                  return {
...
exports.getCategoryView = async (req, res, next) => {
   ...
   try {
      ...
      const menuObj = {
         ...
         c_main_banner:
            menuView.c_main_banner === enumConfig.bannerSizeType.COVER[0]
...
exports.putCategoryUpdate = async (req, res, next) => {
   ...
   try {
      ...
      if (mainBannerFile && mainBannerFile[0]) {
         if (
            menuView.c_main_banner_file !== mainBannerFile[0].path &&
